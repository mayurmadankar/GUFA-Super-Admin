...
const NewDoctors = () => {
  ...
  const { newDoctorList, loading, error } = useSelector(
    (state) => state.doctorUser
  );

  const doctorslist = newDoctorList?.doctors || [];
  ...
  const filteredDoctors =
    newDoctorList?.doctors?.filter((doctor) => {
  ...
  const handleViewDoctor = (doctor_id) => {
    navigate(`/doctor/doctor_detail/${doctor_id}`, {
      state: { showApproveButton: true, showEditButton: true }
    });
  };
          ...
          <tbody>
            {paginatedDoctors?.map((doctor, index) => (
              <tr style={{ cursor: "pointer" }} key={index}>
                ...
                <td>Pending</td>
                <td>
                  {Array.isArray(doctor.consultation_fee)
                    ? doctor.consultation_fee.join(",")
                    : doctor.consultation_fee || "\u20B9 99"}
                </td>
