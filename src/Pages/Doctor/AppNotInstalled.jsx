...
const AppNotInstalled = () => {
  ...
  const handleLAppNotInstView = (doctor_id) => {
  ...
  useEffect(() => {
    const doctors = liveDoctorList?.not_logged_in || [];
    ...
    if (!searchTerm) {
      ...
      const filtered = doctors.filter((doctor) => {
        return (
          ...
          (specialityMap[Number(doctor.speciality)] || doctor.speciality || "")
            .toLowerCase()
            .includes(searchLower) ||
  ...
  return (
    <div className={`table-responsive ${css.approved_doctor_container}`}>
      ...
      <table className="table text-center">
        ...
        ) : paginatedAppNotInstalledDoctors?.length > 0 ? (
          <tbody>
            {paginatedAppNotInstalledDoctors.map((doctor) => (
              <tr key={doctor.doctor_id} style={{ cursor: "pointer" }}>
                ...
                <td>{doctor.number}</td>
                <td>65</td>
                <td onClick={() => handleLAppNotInstView(doctor?.doctor_id)}>
